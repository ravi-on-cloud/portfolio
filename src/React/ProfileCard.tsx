...
const ProfileCard: React.FC = () => {
  ...
  // Gyro motion disabled per request; touch/drag tilt remains enabled.
    ...
    >
      ...
      <div
        ...
        style={{
          padding: "2px",
          background:
            "conic-gradient(from 0deg at 50% 50%, rgba(255,87,87,0.7), rgba(255,186,73,0.7), rgba(255,255,120,0.7), rgba(120,255,160,0.7), rgba(102,255,255,0.7), rgba(120,160,255,0.7), rgba(196,113,237,0.7), rgba(255,87,87,0.7))",
          WebkitMask: "linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0)",
          WebkitMaskComposite: "xor" as any,
          maskComposite: "exclude" as any,
          animation: "spin 6s linear infinite",
          // Lower when idle, stronger when tilted
          opacity: 0.15 + 0.5 * tilt.mag,
        }}
      />
      {/* Outer glow */}
      <div
      ...
      <div
        ...
        style={{
          background:
            "conic-gradient(from 0deg at 50% 50%, rgba(255,87,87,0.5), rgba(255,186,73,0.5), rgba(255,255,120,0.5), rgba(120,255,160,0.5), rgba(102,255,255,0.5), rgba(120,160,255,0.5), rgba(196,113,237,0.5), rgba(255,87,87,0.5))",
          filter: "blur(60px)",
          // Softer at rest, vivid on tilt
          opacity: 0.06 + 0.38 * tilt.mag,
      ...
      {(() => {
        const ox = tilt.ry * 1.6; // horizontal offset follows Y-rotation
        const oy = -tilt.rx * 1.6; // vertical offset opposite X-rotation
        const blur = 40 + 80 * tilt.mag;
        const darkAlpha = 0.50 + 0.30 * tilt.mag;
        const glowBase = 28 + 50 * tilt.mag;
        // Rainbow multi-glow stack
        // Lower base opacity when idle
        const glowR = `0 0 ${glowBase + 10}px rgba(255,87,87,${0.04 + 0.22 * tilt.mag})`;
        const glowO = `0 0 ${glowBase + 8}px rgba(255,186,73,${0.035 + 0.20 * tilt.mag})`;
        const glowY = `0 0 ${glowBase + 6}px rgba(255,255,120,${0.03 + 0.18 * tilt.mag})`;
        const glowG = `0 0 ${glowBase + 12}px rgba(120,255,160,${0.045 + 0.24 * tilt.mag})`;
        const glowC = `0 0 ${glowBase + 14}px rgba(102,255,255,${0.04 + 0.22 * tilt.mag})`;
        const glowB = `0 0 ${glowBase + 16}px rgba(120,160,255,${0.04 + 0.22 * tilt.mag})`;
        const glowP = `0 0 ${glowBase + 18}px rgba(196,113,237,${0.045 + 0.24 * tilt.mag})`;
        ...
        return (
          ...
          >
            ...
            {/* Icon/dot pattern behind avatar */}
            <div
              aria-hidden
              className="absolute inset-x-6 top-20 bottom-28 rounded-2xl overflow-hidden pointer-events-none"
              style={{ transform: `translateZ(12px) translate3d(${parallax.sx * 0.6}px, ${parallax.sy * 0.6}px, 0)` }}
            >
              <div
                className="w-full h-full"
                style={{
                  backgroundImage:
                    "radial-gradient(circle at 1px 1px, rgba(255,255,255,0.03) 1px, transparent 1.4px)",
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-2 w-[90%] md:w-[84%] z-[2]">
        <div className="transform-gpu" style={{ transform: `translateZ(18px) translate3d(${parallax.sx}px, ${parallax.sy}px, 0)` }}>
          <div className="w-full bg-[var(--surface)] border border-[var(--border-muted)] backdrop-blur-2xl rounded-xl px-3 py-2 grid grid-cols-3 place-items-center gap-2 shadow-[0_8px_30px_rgba(0,0,0,0.15)]">
            <a
              href={profile.github}
              target="_blank"
              aria-label="GitHub"
              className="size-9 flex items-center justify-center text-[var(--white-icon)] hover:text-[var(--white)] transition duration-300 ease-in-out border border-[var(--border-muted)] rounded-lg bg-[var(--surface)] hover:bg-[var(--surface-hover)]"
            >
